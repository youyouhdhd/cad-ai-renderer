...
The local package prepares geometry evidence and a production prompt. The host's official image-generation skill or tool performs final image synthesis.
...
Prefer a single multi-output official invocation when supported. Otherwise use four equivalent invocations. Keep prompt and input order constant; natural stochastic variation supplies candidate diversity.
